...
export function NavigationHeader() {
  ...
  const clerkAuth = isAuthDisabled ? null : useAuth();
  const clerkUser = isAuthDisabled ? null : useUser();
  
  const { isSignedIn } = isAuthDisabled ? { isSignedIn: false } : clerkAuth;
  const { user } = isAuthDisabled ? { user: null } : clerkUser;
      ...
      <div className="flex gap-3 items-center">
        ...
        {isSignedIn && (
          <>
            <Link
              href="/dashboard"
              className={`relative capitalize opacity-90 hover:opacity-100 px-2 py-1 rounded ${
            ...
            <Link
              href="/simple-chat"
              className={`relative capitalize opacity-90 hover:opacity-100 px-2 py-1 rounded ${
                pathname === '/simple-chat' ? 'bg-blue-700' : ''
              }`}
            >
              Chat
            </Link>
